...
language_list=c("italian", "english")  
...
```{r get_aoas}
language_list_=c("Italian", "English (American)")  
...
reliabilities <- expand_grid(language = c( "english", "italian"), 
...
reliabilities_aoa <- expand_grid(language = c("English (American)", "Italian"), 
                            word_class = c("all", "nouns","adjectives","verbs",
                                           "function_words","other")) %>% 
  rowwise %>%
...
reliabilities_aoa <- reliabilities_aoa %>%
    mutate(language = sub("English (American)", "english", language))
...
r2 <- expand_grid(lang = c("english", "italian"), 
...
```{r final data}
r2_ <- r2 %>%
  select(language, lexical_class, r2) %>%
