...
```{r clean load data, results='hide', message=FALSE, warning = FALSE}
library(tidyverse)
library(lme4)
library(ggeffects)
library(wordbankr)
...
```{r example, message=FALSE, message=FALSE, warning = FALSE}
...
```{r raw frequency, message=FALSE, warning = FALSE}
...
```{r modeling frequency intercept, message=FALSE, warning = FALSE}
...
```{r example frequency, message=FALSE, warning = FALSE}
...
```{r load wordbank, message=FALSE, warning = FALSE}
...
```{r example load wordbank, message=FALSE, warning = FALSE}
...
```{r reliability frequency, message=FALSE, warning = FALSE}
...
```{r example reliability frequency, message=FALSE, warning = FALSE}
...
```  
French nouns:   `r r_fr_nouns`, `r sbformula(r_fr_nouns)` 
French adjectives: `r r_fr_adj`, `r sbformula(r_fr_adj)`
French verbs: `r r_fr_verbs`, `r sbformula(r_fr_verbs)` 
French other: `r r_fr_other`,`r sbformula(r_fr_other)`
French function words: `r r_fr_fw`,`r sbformula(r_fr_fw)`
  
Italian nouns:   `r r_it_nouns`, `r sbformula(r_it_nouns)` 
Italian adjectives: `r r_it_adj`, `r sbformula(r_it_adj)`
Italian verbs: `r r_it_verbs`, `r sbformula(r_it_verbs)` 
Italian other: `r r_it_other`,`r sbformula(r_it_other)`
Italian function words: `r r_it_fw`,`r sbformula(r_it_fw)`

### Reliability_frequency: cronebach's alpha - treat each word as item and each child as subject

```{r reliability alpha, message=FALSE, warning = FALSE}
...
```{r example reliability alpha, message=FALSE, warning = FALSE}
...
French nouns a:   `r r_fr_nouns_a`, `r sbformula(r_fr_nouns_a)` 
French adjectives a: `r r_fr_adj_a`, `r sbformula(r_fr_adj_a)`
French verbs a: `r r_fr_verbs_a`, `r sbformula(r_fr_verbs_a)` 
French other a: `r r_fr_other_a`,`r sbformula(r_fr_other_a)`
French function words a: `r r_fr_fw_a`,`r sbformula(r_fr_fw_a)`

Italian nouns a: `r r_it_nouns_a`,`r sbformula(r_it_nouns_a)`

### Reliability_AoA:
  
```{r reliability aoa, message=FALSE, warning = FALSE}  
...
```{r example reliability aoa, message=FALSE, warning = FALSE}  
...
```{r merge aoa and childes, message=FALSE, warning = FALSE}
...
```{r example merge aoa and childes it, message=FALSE, warning = FALSE}
...
```{r example plots, message=FALSE, warning = FALSE}
