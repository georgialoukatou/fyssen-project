...
mergeAoaChildes<- function(lang_,clas_){
  if (clas_ != "all"){
                 aoas <- aoas %>% filter(lexical_class==clas_)}
  dataAoa <-  aoas %>% filter(language==lang_)
  if (lang_ == "English (American)"){
                 lang_ <- "English"}
  corpus_frequency_ <- frequencies %>% filter(language==lang_)
  wblemmas<-unique(dataAoa$lemma) #unique wordbank lemmas
...
for (lang_ in language_list){
  for (clas_ in word_class){
   nam <- paste(lang_, clas_, sep = "_")
   nam <- gsub(" ", "", nam, fixed = TRUE)
   nam <-noquote(nam)
   df <- mergeAoaChildes(lang_, clas_)
   regress=regression_option1(df)
   df <- df %>% mutate(r2 = regress)
   list_[[nam]] <-df
  }
}
  return(list_)
...
```{r example merge aoa and childes exam}
...
plot_frequency3<-function(db){
  ggplot(db, 
    ...
    geom_point() + 
    geom_smooth(method = "lm") +   facet_wrap(~lexical_category, nrow=2)
}

plot_frequency1(dat$`English(American)_all`)
plot_frequency2(dat$`English(American)_all`)
plot_frequency3(dat$`English(American)_all`)
...
```{r final plot}
EnglishA_all<-unique(dat$`English(American)_all`$r2)
