...
corpus_frequency_short <- corpus_frequency %>% ungroup() %>% select(lemma, language, FrequencyLogMean, intercept) #get only important columns 
...
CHILDES_WB_short <- CHILDES_WB_short%>% select(lemma, FrequencyLogMean, intercept, lexical_category, aoa, num_item_id) 
...
ggplot(CHILDES_WB_short, aes(intercept, aoa, label=lemma)) + geom_point()  +geom_text(aes(label=lemma),hjust=0, vjust=0) +xlim(0,0.5) + facet_wrap(~lexical_category, nrow=2)
...
option1<-lmer(aoa~ intercept + (1|lexical_category), data=CHILDES_WB_short) 
#number_producing ~ age + frequency 
anova(option1)
ggpredict(option1, c("intercept")) %>% plot()
